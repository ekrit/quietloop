# car scraper — research & plan

Goal: scrape car listings daily, for a curated set of brands, filtered to
...
the source site directly (confirmed 403 at the proxy level) — but GitHub Actions runners
...
Confirmed via search-indexed URLs (real search links that Google/Bing had
crawled):

- Cars live under `category_id=18` (top-level "Automobili"): `/pretraga?category_id=18`
...
- There is a separate, apparently newer/related listing surface at the site's
  `/vozila` path (differently branded — new/used vehicles, cars, motorcycles,
  trucks) — worth checking whether this is the same backend as
  `/pretraga?category_id=18` or a distinct vertical with its own structure.
- An **official platform API** exists at a documented subdomain, with `Listings`,
  ...
  listings onto the platform, e.g. from a dealer's inventory system) rather than
  a public search API for arbitrary third-party queries — but this needs direct
...
1. Fetch the site's `robots.txt` and read it — don't assume permissive rules.
...
5. Check whether the platform API's listing-read endpoints work without
...
```jsonc
{
  "id": "12345678",                  // (✓) source site's numeric listing id
  "url": "https://example.com/artikal/...", // NOT CONFIRMED -- best-effort guess, see §1a
  ...
  "brand_id": 42, "category_id": 18, "city_id": 7,   // (✓) source site's own numeric ids
  "condition_raw": "used",            // (✓) source site's own condition string, unmapped
  ...
  "published_date": "2026-07-20",     // source site's own publish date — the true
...
- **Days on market (`days_listed`)** — the headline metric. Computed from
  `published_date` (source site's own timestamp, not our scrape date) to the day the
...
`brand=64` for Opel, seen in a search-indexed URL (§1), though
...
5. **Fails the job at the end if the scraper didn't exit 0** (see the
   ...
   the source site started blocking the requests), separate from whether the
...
**Category IDs** (real, from a live "Debug fetch" run against the source
site's search endpoint's `state.search.aggregations.categories`, one
...
category IDs confirmed via live "Debug fetch" runs against the source
site's search endpoint (`?trazilica=<term>`)'s
...
this, checked whether the site exposes a structured brand field to lean on

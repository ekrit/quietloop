"""Runtime configuration for the car scraper.

Values flagged NEEDS VERIFICATION are unconfirmed guesses; values flagged
CONFIRMED (NOT) are backed by an actual live run against the source site via
the "Debug fetch" GitHub Actions workflow (this sandbox itself can't reach the
...
class Brand:
    name: str
    # Prefer a numeric `brand=<id>` filter once confirmed (precise).
